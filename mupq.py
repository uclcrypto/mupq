...
class Implementation(object):
    ...
    def __init__(
        self, project, primitive, scheme, implementation, path, namespace, extraflags=[]
    ):
        """Sets up this scheme"""
    ...
    def run_make(self, target):
        makeflags = ["make", f"IMPLEMENTATION_PATH={self.path}"]
        if self.namespace is not None:
            makeflags.append(f"MUPQ_NAMESPACE={self.namespace}")
        makeflags.extend(self.extraflags)
        makeflags.append(target)
        return subprocess.check_call(makeflags, stdout=sys.stdout, stderr=sys.stderr)
                ...
                for scheme in os.listdir(scheme_folder):
                    ...
                    for implementation_path in os.listdir(scheme_path):
                        ...
                        if not os.path.isdir(path):
                            continue
                        impl = Implementation.from_path(
                            parent, path, namespace, self.makeflags
                        )
...
class BoardTestCase(abc.ABC):
    ...
    @abc.abstractmethod
    def run_test(self, implementation, **kwargs):
        implementation.build_binary(
            f"{self.test_type}", self.platform_settings.binary_type
        )
        binary = implementation.get_binary_path(
            f"{self.test_type}", self.platform_settings.binary_type
        )
        return self.interface.run(binary, **kwargs)
        ...
        for implementation in self.get_implementations():
            ...
            if (
                ...
                and (
                    ...
                    and implementation.scheme + "/" + implementation.implementation
                    not in args
                )
            ):
                continue
...
class SizeBenchmark(StackBenchmark):
    ...
    def run_test(self, implementation):
        ...
        output = subprocess.check_output(
            self.platform_settings.size_executable
            + " -t "
            + implementation.get_library_path(),
            shell=True,
...
class TestVectors(BoardTestCase):
    ...
    def run_test(self, implementation):
        output = (
            super()
            .run_test(implementation, expected=self.testvector[implementation.scheme])
            .encode("utf-8")
        )
        checksum = self.hash_output(output)
        assert self.testvectorhash[implementation.scheme] == checksum

    def run_make(self, target, path, namespace):
        makeflags = ["make", f"IMPLEMENTATION_PATH={path}"]
    ...
    def _prepare_testvectors(self, args):
        for scheme, implementations in self.schemes.items():
            for impl in implementations:
                ...
                self.testvector[impl.scheme] = output
                break
    ...
    def test_all(self, args):
        ...
        for implementation in implems:
            self.log.info(f"Start test implem {implementation}")
            self.run_test(implementation)
            self.log.info(f"Passed test for implem {implementation}")
...
class BuildAll(BoardTestCase):
    ...
    def run_test(self, implementation):
        for test_type in ("test", "testvectors", "speed", "hashing", "stack"):
            implementation.build_binary(test_type, self.platform_settings.binary_type)
...
class Converter(object):
    ...
    def _hashing(self):
        ...
        self._processPrimitives(
            "benchmarks/hashing/crypto_kem/", "hashing", "crypto_kem"
        )
        ...
        self._processPrimitives(
            "benchmarks/hashing/crypto_sign/", "hashing", "crypto_sign"
        )
    ...
    def _processPrimitives(self, path, benchmark, type_):
        ...
        for scheme in sorted(os.listdir(path)):
            ...
            for implementation in sorted(os.listdir(path + "/" + scheme)):
                measurements = []
                for measurement in os.listdir(
                    path + "/" + scheme + "/" + implementation
                ):
                    with open(
                        path + "/" + scheme + "/" + implementation + "/" + measurement,
                        "r",
                    ) as f:
    ...
    def _parseData(self, fileContents, benchmark, type_):
        ...
        if benchmark == "size":
            ...
            if type_ == "crypto_kem":
                ...
                encsign = (
                    int(parts[parts.index("encaps hash cycles:") + 1]) / encsigntotal
                )
                decverifytotal = int(parts[parts.index("decaps cycles:") + 1])
                decverify = (
                    int(parts[parts.index("decaps hash cycles:") + 1]) / decverifytotal
                )
            else:  # crypto_sign
                encsigntotal = int(parts[parts.index("sign cycles:") + 1])
                encsign = (
                    int(parts[parts.index("sign hash cycles:") + 1]) / encsigntotal
                )
                decverifytotal = int(parts[parts.index("verify cycles:") + 1])
                decverify = (
                    int(parts[parts.index("verify hash cycles:") + 1]) / decverifytotal
                )
    ...
    def _formatData(self, scheme, implementation, data, benchmark):
        if benchmark == "speed":
            ...
            encsign = self._formatPercentage(
                statistics.mean([item[1] for item in data])
            )
            decverify = self._formatPercentage(
                statistics.mean([item[2] for item in data])
            )
            self._row([scheme, implementation, keygen, encsign, decverify])
        elif benchmark == "size":
...
class MarkdownConverter(Converter):
    ...
    def _formatStats(self, l):
        ...
        return "AVG: {:,} <br /> MIN: {:,} <br /> MAX: {:,}".format(
...
class CsvConverter(Converter):
    ...
    def _speed(self):
        ...
        cyclesKem = self._processPrimitives(
            "benchmarks/speed/crypto_kem/", "speed", "crypto_kem"
        )
        ...
        cyclesSign = self._processPrimitives(
            "benchmarks/speed/crypto_sign/", "speed", "crypto_sign"
        )
